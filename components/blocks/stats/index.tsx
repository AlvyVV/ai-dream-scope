  ...
  return (
    <section id={section.name} className="py-16">
      <div className="container flex flex-col items-center gap-4">
        ...
        <div className="w-full grid gap-10 md:grid-cols-4 lg:gap-0 mt-8">
          {section.items?.map((item, index) => {
            return (
              <div key={index} className="text-center">
                <p className="text-lg font-semibold text-muted-foreground">{item.title}</p>
                <p className="pt-2 text-5xl font-semibold lg:pt-4 text-primary">{item.label}</p>
                <p className="text-xl mt-2 font-normal text-muted-foreground">{item.description}</p>
              </div>
            );
          })}
        </div>

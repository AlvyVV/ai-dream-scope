...
export default function Pricing({ pricing }: { pricing: PricingType }) {
  ...
  const handleCheckout = async (item: PricingItem, cn_pay: boolean = false) => {
    try {
      ...
      // 对于非中国支付，提示Stripe已禁用
      if (!cn_pay) {
        toast.error('Stripe支付功能已临时禁用，请使用其他支付方式');
        setIsLoading(false);
        setProductId(null);
        return;
      }
      ...
      */
      ...
      // 仅支持中国支付方式，直接跳转到成功页面
      window.location.href = `/pay-success/${session_id}`;
    } catch (e) {
    ...
    <section id={pricing.name} className="py-16">
      <div className="container">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div className={`md:min-w-96 mt-0 grid gap-6 md:grid-cols-${pricing.items?.filter(item => !item.group || item.group === group)?.length}`}>
            {pricing.items?.map((item, index) => {
              ...
              return (
                <div key={index} className={`rounded-lg p-6 ${item.is_featured ? 'border-primary border-2 bg-card text-card-foreground' : 'border-muted border'}`}>
                  <div className="flex h-full flex-col justify-between gap-5">
                    <div>
                      ...
                      <div className="flex items-end gap-2 mb-4">
                        {item.original_price && <span className="text-xl text-muted-foreground font-semibold line-through">{item.original_price}</span>}

...
export async function POST(req: Request) {
  try {
    ...
    */
    ...
    // 创建模拟会话ID
    const stripe_session_id = `mock_cnpay_${order_no}`;
    // 创建模拟订单详情
    const orderOptions = {
      metadata: {
        project: process.env.NEXT_PUBLIC_PROJECT_NAME || '',
      ...
      mode: is_subscription ? 'subscription' : 'payment',
      success_url: `${process.env.NEXT_PUBLIC_WEB_URL}/pay-success/${stripe_session_id}`,
      cancel_url: cancel_url,
    };
    const order_detail = JSON.stringify(orderOptions);

    await updateOrderSession(order_no, stripe_session_id, order_detail);

    return respData({
      public_key: process.env.STRIPE_PUBLIC_KEY || 'mock_key',
      order_no: order_no,
